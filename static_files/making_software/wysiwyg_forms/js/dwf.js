define(function (require, exports, module) {
    ...
    controlPanel.activate(base, {
        addField: function (fieldType, widget) {
            util.prompt("Nome do campo:", function (label) {
                if ( label ) {
                    if ( form.getFieldByLabel(label) ) {
                        util.prompt("Ja existe um campo com esse nome. Escolha outro nome:",
                                    arguments.callee);
                    }
                    else {
        ...
        addChoice: function (fn, ctx) {
            util.prompt('Nome da opção:', function (label) {
                if ( label ) {
                    if ( form.activeField.getChoiceByLabel(label) ) {
                        util.prompt("Ja existe uma opção com esse nome. Escolha outro nome:",
                                    arguments.callee);
                    }
                    else {
    ...
    transactions.startAutoSaveLoop((function () {
        ...
        return {
            ...
            error: function (e) {
                controlPanel.enableSave();
                messages.removeMessage(msgId);
                messages.warn('Houve um erro ao salvar.');
                if ( e ) {
                    console.error('Codigo de erro: ' + e);
                }
            }
        };
    }()));
