...
class ApplyTransactions(OverrideSaveMixin, LoginRequiredMixin, DetailView):
    """
...
class Edit(OverrideSaveMixin, LoginRequiredMixin, DetailView):
    """
    ...
    def get_object(self, queryset=None):
        try:
            ...
            #logger.error(self.request.user)
                ...
                #logger.error(self.kwargs['id'])
                #logger.error(self.kwargs['pai'])
                
            name = "Título do formulário "
            name += self.kwargs['elemento_nome'].encode('utf-8')
            #desc = "Descrição do formulário. Você pode alterar o título e a descrição nas Configurações do Formulário, ao lado."
            desc = "Essa descrição não é apresentada no seu sistema. Se você quiser pode utilizá-la para armazenar algum tipo de instrução referente a esse formulário para o seu próprio uso."
