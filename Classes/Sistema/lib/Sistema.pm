...
sub new {
	my $class = shift;
    my $id = shift;
    my $inteliform_perl_dir = shift;
    
	my $self = {};
	bless $self, $class;
    $self->db_connection($inteliform_perl_dir);
	$self->initialize($id);
	return $self;
}
...
sub db_connection {
    ...
    $self->{'dbh_inteliform'} = DBI->connect("dbi:Pg:dbname=loopware;host=$config{postgres_host}",
...
sub RecuperaRelacionamentos{
	...
	if ($self->{'dbh_inteliform'}) {
	   my $sth = $self->{'dbh_inteliform'}->prepare($query);
